...
class schooldayController extends Controller
{
    ...
    public function learner(Request $request){
        ...
        $result = DB::select('SELECT strLearCode, CONCAT(strLearLname,", ",strLearFname) AS strLearName FROM tblLearner WHERE intLearSesId = ? AND blLearDelete = 0 AND strLearCode NOT IN (SELECT strAttLearCode FROM tblAttendance WHERE intAttSDId = ?) ORDER BY strLearLname', [$sesid,$sdid]);
